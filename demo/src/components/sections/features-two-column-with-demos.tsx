...
export function Feature({
  ...
  return (
    <div className={clsx('h-full rounded-2xl bg-mist-950/2.5 p-2 dark:bg-white/5', className)}>
      <div className="relative overflow-hidden rounded-lg [&_img]:outline [&_img]:-outline-offset-1 [&_img]:outline-black/10 dark:[&_img]:outline-white/10">
        {demo}
      </div>
...
export function FeaturesTwoColumnWithDemos({
  ...
  const animatedFeatures = Children.map(features, (child, index) => {
    ...
    return (
      <div
        className={clsx(
          'h-full transition-[translate,opacity] duration-700 ease-out',
  ...
  return (
    <Section stickyEyebrow={stickyEyebrow} sectionHue={sectionHue} {...props}>
      ...
      <div
        ...
        className={clsx(enableHorizon && 'section-horizon pt-6')}
      >
