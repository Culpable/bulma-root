...
- `site/package.json` and `site/pnpm-lock.yaml` own installed packages, versions, and available scripts.
- `site/astro.config.mjs` owns Astro output, canonical origin, URL policy, integrations, and adapter configuration.
- `site/src/pages/` owns route entrypoints, `site/src/layouts/` owns document shells, and `site/src/components/` owns reusable presentation and document fragments.
...
- Keep route metadata inputs with pages or content entries, resolve them through `site/src/lib/metadata.ts`, and render them once through the base layout's shared head component.
- Put build-processed component assets in `site/src/`. Use `site/public/` only for stable unprocessed URLs or root files.
...
- A setting in `site/astro.config.mjs` chosen to satisfy a security policy, host, or provider must record which requirement it serves and what else it affects.
