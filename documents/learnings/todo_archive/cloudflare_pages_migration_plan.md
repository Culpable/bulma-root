> Archived 2026-09-05. Superseded by `documents/todo/astro_workers_migration_plan.md`. The Astro Workers migration replaced the proposed Cloudflare Pages move. All approval and stop instructions below are historical.
...
- Completed Steps 1 through 5F and the first full Step 5G hosted proof run. Step 5G remains `IN PROGRESS` because REQ-27 failed and the Sydney release gate is blocked and incomplete.
- Completed the requested performance implementation, exact-artifact GitHub and Cloudflare preview deployment, body parity, full local and hosted Lighthouse matrices, fixed-probe regional matrix, hosted browser matrix, and forced analytics replay.
- Kept the aggregate plan status `IN PROGRESS`. Steps 6 through 10 remain blocked until a later Step 5G run passes every gate and the user explicitly approves cutover.
...
- Committed and pushed the Steps 5D-5F implementation as `3b131b8bd02c9cf29fe9f0396bcbb1cecfd78416`. GitHub Actions run `33321153005` succeeded and deployed GitHub Pages.
- Downloaded that workflow's exact `github-pages` artifact, SHA-256 `ae117694ca17e55eb12be7de0d1b2b04418001cdfd286eded3df82cabd461a17`, and uploaded it without rebuilding to Cloudflare preview deployment `8eafed9f-a947-4b88-9449-b2d09f58c29a` at `https://8eafed9f.bulma-root.pages.dev/`.
- Verified the Cloudflare deployment reports the exact clean commit and `uses_functions: false`. It remains a noindexed preview with no custom domain or production deployment.
- Compared the 179-file hosted manifest with both hosts. Every decoded body, status, content type, canonical, discovery file, and unknown-path response matched the exact artifact. Expected provider header differences passed.
- Completed 150 standard Lighthouse reports and 30 separate DevTools-throttled mobile reports across both hosts, all five routes, and both viewport modes. Every REQ-19 mobile route ceiling passed; About image-delivery waste was `13.36 KiB`, below the REQ-22 `20 KiB` ceiling.
- Recorded current mobile GitHub / Cloudflare score and LCP medians: `/` `93.5 / 2,851 ms` versus `93 / 2,947 ms`; `/about/` `96 / 2,551 ms` versus `96.5 / 2,628 ms`; `/pricing/` `95 / 2,777 ms` versus `97 / 2,499 ms`; `/contact/` `96.5 / 2,551 ms` versus `96 / 2,570 ms`; `/privacy-policy/` `97 / 2,515 ms` versus `97 / 2,457 ms`.
- Applied REQ-27 against each host's Step 5C baseline. Cloudflare passed all five routes. GitHub failed all five because FCP, LCP, Speed Index, and performance score exceeded the fixed allowances on every route. Deterministic compressed JavaScript decreased on all five routes, but the gate does not permit that fact to override the hosted regression result.
- Re-ran 240 fixed-probe Globalping requests. All returned HTTP 200. Melbourne medians were GitHub `222 ms` and Cloudflare `41.5 ms`; Sydney medians were GitHub `13 ms` and Cloudflare `34 ms`; combined medians were GitHub `209 ms` and Cloudflare `38 ms`. These results describe transport, not regional rendering.
- Confirmed IPv4 and IPv6 access on both hosts. GitHub's observed IPv6 path used an IPv4-mapped address; Cloudflare returned native IPv6.
- Re-queried Cloudflare Speed API quota and Sydney availability. One GitHub homepage test completed, but the preserved corresponding Cloudflare `pages.dev` create request returned HTTP 500 with error `1004`, `speed.errors.generic`. Stopped instead of running a prohibited partial GitHub-only matrix. The required 50-test same-runner Sydney gate therefore remains blocked and incomplete.
- Re-ran the hosted browser matrix on every route and both viewports for both hosts. Permanent dark rendering under light scheme emulation, overflow, navigation, both `#lenders` paths, rapid FAQ reversal, pricing and keyboard states, equal desktop card heights, mobile menu, contact error and success paths, responsive About sources, and first-party request checks passed.
- Forced sampled and unsampled analytics profiles on both hosts with all third-party traffic intercepted. Page Views, referral data, custom events, first-touch values, and the recorder sampling boundary matched. The intercepted recorder fixture proves the load boundary; Node tests cover the internal analytics contract.
- Updated `documents/guides/_hosting.md` and the existing cold-reader report with the full result and preserved evidence. `_animations.md` remains synchronised because the final implementation and hosted checks did not change the documented animation behaviour.
- Recorded a concurrent post-measurement GitHub Pages deployment for child commit `4a005a64b8b44b91d168602049cbef38867f79be`. It changes only the `llms.txt` source, generator, generated output, and tests, so it does not change the measured browser routes or failed-gate decision. It ends the live byte-matched state and requires any future approval run to publish a new exact pair.
- Kept Step 5G and the aggregate migration `IN PROGRESS`. Recommended keeping GitHub Pages in production. No DNS, custom domain, Cloudflare production deployment, redirect, GitHub Pages setting, or post-evidence Git push changed.
- Recorded GitHub artifact `9743765230` from workflow `33351476104` as the current exact-pair candidate for a later Step 5G rerun. If it expires, the rerun must select and record the next successful current GitHub artifact before deploying the unchanged bytes to `cloudflare-comparison`.
- Published that exact current artifact without rebuilding to clean preview deployment `3712aa0a-8883-4c46-bf42-3dc1e46be404` at `https://3712aa0a.bulma-root.pages.dev/`. The deployment reports the exact clean commit, branch `cloudflare-comparison`, `uses_functions: false`, no custom domain, and success.
- Verified all 179 served artifact files against both hosts with zero decoded-body, status, or normalised-media-type mismatches. Cloudflare parsed the artifact's `_headers` control file.
- Re-ran lint, production build, all 33 tests, and deterministic performance budgets against the current revision. All passed.
- Completed a new exact-pair Lighthouse matrix with 100 mobile and 50 desktop reports. REQ-27 again passed all five Cloudflare routes and failed all five GitHub routes against the locked Step 5C host baseline. Every GitHub route exceeded FCP and LCP allowances, four exceeded Speed Index, and every route lost more than two performance-score points.
- Re-queried the Cloudflare Speed API before starting the Sydney matrix. It reported 49 of 50 free tests remaining and Sydney available. The plan requires at least 50 tests before starting, so no test was consumed and the 50-test same-runner gate remains blocked.
- Stopped before the separate DevTools, Globalping, hosted-browser, and analytics reruns because REQ-27 failed and the Sydney gate remained blocked. No production, DNS, custom-domain, GitHub Pages, branch, commit, or push action was performed.
- Stored the new parity summary, Lighthouse medians, REQ-27 calculation, and 150 raw reports in the canonical cold-reader report under `evidence/step5g-current-20260831/`.
- Continued Step 5G on 4 September 2026 with the Lighthouse Test workflow against the same five-route exact pair. Lighthouse `13.4.1` and Chrome `152.0.7977.76` produced 100 calibrated mobile scoring reports, 30 DevTools-throttled mobile diagnostics, 50 desktop reports, and one CPU calibration report with zero runtime errors.
- The host benchmark index was `3050` and selected a `10.5x` mobile slowdown. This profile is valid for a fresh cross-host comparison but is not directly comparable with the older fixed-4x REQ-27 baseline, so it does not replace that gate calculation.
- Current mobile GitHub / Cloudflare score and LCP medians were: `/` `91 / 2,874 ms` versus `90 / 2,307 ms`; `/about/` `98 / 2,018 ms` versus `96.5 / 2,070 ms`; `/pricing/` `95 / 2,782 ms` versus `98 / 1,857 ms`; `/contact/` `99 / 1,529 ms` versus `99 / 1,925 ms`; `/privacy-policy/` `99 / 1,896 ms` versus `97.5 / 1,902 ms`. Desktop scores remained `99-100`.
- Reconfirmed decoded content parity for all five public routes and the three discovery/readability files. The Mixpanel recorder sampled one GitHub homepage run and six Cloudflare homepage runs, so the official all-run homepage score and TBT medians are conservative for Cloudflare; the complete evidence records both all-run and unsampled diagnostics.
- Retried the Sydney Speed API only after it reported all 50 tests available. The first Cloudflare preview request again failed with error `1004`, `speed.errors.generic`, and consumed no quota. Stopped before a partial GitHub-only matrix.
- Kept Step 5G and the aggregate migration `IN PROGRESS`. The Sydney same-runner gate remains blocked, the fixed REQ-27 gate remains failed, and no production, DNS, custom-domain, GitHub Pages, branch, commit, or push action was performed. Evidence is stored at `/Users/sacino/Documents/codex/web-performance/bulma-hosting/step5g-speed-20260904/`.
