...
- **Workers Builds fallback:** if the GitHub App grant cannot be completed in the session, finish every local and bootstrap step, upload a preview version, and stop before Step 8 with the exact grant URL; do not substitute GitHub Actions without the user's decision.
...
- "Everything pre cutover should be handled by the agent, who has access." All Cloudflare and GitHub work is agent-run through existing credentials; the user is involved only for the GitHub App repository grant and the cutover approval.
...
- **Assumptions:** the Cloudflare GitHub App is installed for `Culpable` with selected repositories; granting `bulma-root` is a one-click GitHub step the agent opens with `open '<url>'`.
- **Reconsider when:** the grant is refused or Builds cannot be configured by API.
...
- Open `https://github.com/settings/installations` with `open` and ask the user to grant `Culpable/bulma-root` to the Cloudflare Workers and Pages app; poll `GET /accounts/{account_id}/builds/repos/providers/github/...` (or the current repository-listing endpoint from `deploy-cloudflare-workers-sites/references/builds-versions-and-release-control.md` and the Builds API reference) until the repository is visible.
- Create the repository connection and two triggers per REQ-19 with environment variables `NODE_VERSION=22.23.1` and `PNPM_VERSION=11.22.0`; trigger a production build for the current `main` and a preview build for a throwaway branch only if the user authorises the branch; otherwise verify the production trigger with the next authorised push.
...
- Commit the `site/` work and documentation changes on `main` under `<git_rules>` and push only when the execution request authorises the push; wait for the Workers Builds production build to succeed and confirm the deployed version.
