...
- **REQ-9 (MUST):** Metadata parity per route: identical `<title>` strings, descriptions, self-referencing canonicals, Open Graph and Twitter values, `og:locale en_AU` (user-approved exception below), absolute `og:image`, `rel="describedby"` llms link, and dns-prefetch links, emitted once by `PageMetadata.astro` from `site/src/config/site.ts`, `site/src/lib/metadata.ts`, and per-page inputs.
  - **User-approved locale drift (2026-09-05):** Change only Open Graph `og:locale` from production's inherited `en-AU` to `en_AU`. Open Graph requires `language_TERRITORY`; preserving the hyphen would preserve a production defect. The user explicitly approved this parity exception. `site/test/production-parity.test.ts` asserts exactly one `en_AU` on all six built documents and documents the divergence; the captured production baseline is unchanged. The separate BCP 47 HTML `lang` is untouched (the current Astro layout emits `en`). `demo/` and the live GitHub Pages locale remain unchanged until a separately approved cutover.

  - **Recorded drift (2026-09-04, approved):** production's Next.js layout never overrides `openGraph` per route, so every subpage repeats the homepage `og:title`, `og:description`, and `og:url` while its `<meta name="description">` and `<title>` are correct. The Astro site emits per-route `og:title`, `og:description`, and a self-referencing `og:url` instead. This is a deliberate correction of a production defect, not a parity failure; reverting it would republish wrong social metadata on four routes. `site_name`, `image`, `image:alt`, and every Twitter value match production exactly; `locale` follows the later user-approved exception above.
  - **Additive only (2026-09-04, approved):** the Astro head also emits `og:image:type` and `twitter:image:alt`, which production omits. Both are standard, additive, and change no existing value.
...
- **REQ-11 (MUST):** Discovery parity: `/robots.txt` byte-identical to production; `/sitemap.xml` publishing the same five URLs with no `lastmod` and the home page first; `/llms.txt` byte-identical to production, generated from a TypeScript port of `demo/src/lib/llms.js` by `site/src/pages/llms.txt.ts` with the generator's validation rules kept as a Node test.
  - **Amended 2026-09-04, user decision:** the sitemap byte-identical rule is withdrawn. The shared renderer in `site/src/lib/sitemap.ts` sorts URLs and ends the document with a newline, so the emitted file lists `/contact/` before `/pricing/` and is 417 bytes against production's 416. The home page is still first and the URL set is unchanged. Sitemap URL order carries no crawler meaning, and site-specific ordering code to reproduce the previous host's hand-written order was judged unnecessary complexity. `site/test/production-parity.test.ts` asserts the URL set, the leading home page, uniqueness, and the absence of `lastmod` instead of a byte hash.
...
- **REQ-16 (MUST):** `site/public/_headers` provides: `/*` security baseline (`Content-Security-Policy` with build-generated `sha256` hashes for Astro's inline island scripts, `script-src 'self'` plus the Mixpanel recorder origin, `connect-src 'self'` plus Mixpanel and Formspree origins, `form-action 'self' https://formspree.io`, `style-src 'self' 'unsafe-inline'`, `img-src 'self' data:`, `font-src 'self'`, `frame-ancestors 'none'`, `base-uri 'self'`, `object-src 'none'`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-Frame-Options: DENY`, `Permissions-Policy` as in `taxgenie-root`), `/llms.txt` charset, `/_astro/*` `Cache-Control: public, max-age=31536000, immutable`, `https://staging.bulma.com.au/*` and `https://:version.:subdomain.workers.dev/*` `X-Robots-Tag: noindex`. No HSTS. Exact third-party origins come from intercepted network traffic, not memory.
  - **Recorded addition (2026-09-04):** a `/robots.txt` `Content-Type: text/plain; charset=utf-8` rule was added beside the `/llms.txt` rule. Production serves `robots.txt` with the charset; the Worker was serving it without.
...
**Functional:**
- [x] All five routes, `/404.html`, `/robots.txt`, `/sitemap.xml`, `/llms.txt` exist in `site/dist` and match the parity gates, with the amended REQ-11 sitemap rule. All are asserted by `site/test/production-parity.test.ts`.
...
**Post-implementation verification (2026-09-04):** Two corrections against REQ-1 and REQ-12.
- `@fontsource-variable/inter@5.3.0` and `@fontsource-variable/mona-sans@5.3.0` were installed as runtime dependencies but never imported anywhere in `site/`. D-17 resolves both faces through `fontProviders.google()`, so the Fontsource packages were dead weight outside the REQ-1 dependency list. Both were removed and `pnpm-lock.yaml` was regenerated. The emitted `dist/_astro/fonts/*.woff2` files stay byte-identical to the production Next.js font binaries.
- `site/public/scripts/referral-tracking.js` differed from `demo/public/scripts/referral-tracking.js` in six trailing-whitespace runs, breaking the REQ-12 byte-identical rule. The file was re-copied verbatim and the equality is now asserted in `site/test/production-parity.test.ts` for as long as `demo/` exists.
...
**Post-implementation verification (2026-09-04):** An independent re-verification of Steps 1-8 against the live staging Worker found five parity defects that the Step 3 success criteria assert but no automated check enforced. The hosted proof only compared staging against `site/dist`, never `site/dist` against production, so all five passed unnoticed. Four are now fixed and covered by `site/test/production-parity.test.ts`:

| Defect | Production | Shipped staging | Fix |
| --- | --- | --- | --- |
| `og:locale` missing (REQ-9) | `en-AU` on every route | Absent on every route | Added `openGraphLocale: 'en-AU'` to `site/src/config/site.ts` and emitted it from `PageMetadata.astro` |
| `og:site_name` missing (REQ-9) | `Bulma` on every route | Absent on every route | Emitted `site.name` from `PageMetadata.astro` |
| Viewport lost `initial-scale=1` | `width=device-width, initial-scale=1` | `width=device-width` | Restored the full production value in `BaseLayout.astro` |
| `og:image:alt` copy changed (REQ-27) | `Bulma: AI Assistant for Australian Mortgage Brokers` | `Bulma AI assistant for Australian mortgage brokers` | Restored the production string in `site/src/config/site.ts` |

The fifth defect was `sitemap.xml`, which the shared renderer alphabetises and terminates with a newline, so it lists `/contact/` before `/pricing/` and runs 417 bytes against production's 416. The user reviewed it and withdrew the byte-identical rule: the home page is first, the URL set is unchanged, sitemap order carries no crawler meaning, and site-specific ordering code was judged unnecessary complexity. See the REQ-11 amendment.
...
**Post-implementation verification (2026-09-04):** Internal link targets lost their trailing slash, breaking REQ-22's identical-link-target gate.

The Next.js build set `trailingSlash: true`, which rewrites every internal `href` at build time. `demo/src` therefore writes `href="/contact"` and production ships `href="/contact/"`. Astro emits a JSX `href` verbatim, so the faithful component port shipped the raw `/contact`. Staging served 6 unslashed `/contact` and 2 unslashed `/pricing` links per page against production's inverse ratio, and every desktop navigation click paid a `307` redirect to the slashed URL.

The replacement table in Section 4.3 covers `next/link` as "plain `<a>` with identical class strings" but never records that `next/link` also inherited the config's trailing-slash rewrite. That omission is the root cause; a per-file port could not have caught it.

Fixed with one owner: `site/src/lib/internal-href.ts` exports `resolveInternalHref`, applied in all ten anchor-rendering components (`link.tsx`, `button.tsx` x3, `transition-link.tsx` x2, `announcement-badge.tsx`, `glass-press-button-link.tsx`, `precision-porcelain-button-link.tsx`, `navbar-links.tsx`, and the three footer sections). It rewrites only root-relative document paths and leaves external URLs, `mailto:`, `tel:`, protocol-relative URLs, fragments, query strings, and file paths with an extension untouched.

The result is better than production, which still emits one unslashed `/contact` and one unslashed `/pricing` per page because Next's rewrite missed the mobile-menu links. Total link counts now match production exactly on every route (27, 16, 22, 15, 15) with zero redirect hops. `site/test/production-parity.test.ts` asserts no built document links to a route path without its trailing slash, and `site/test/parity.spec.ts` normalises the production manifest side of its href comparison so the gate still catches a genuinely lost or retargeted link.
...
### ~~Step 10: Decommission GitHub Pages, `demo/`, and the Cloudflare Pages resources~~ ✅ **COMPLETED**
...
| `site/test/production-parity.test.ts` (Node, after build) | `dist` against `production-baseline.json` | `robots.txt` and `llms.txt` byte-identical to production; `sitemap.xml` publishes the five production URLs with the home page first, no duplicates, and no `lastmod`; every document declares the production viewport, `og:site_name`, `og:locale`, and `og:image:alt`; `referral-tracking.js` byte-identical to its `demo/` source |
...
## Open Graph locale correction (2026-09-05)

- [x] Reproduce the inherited locale defect with the corrected parity assertion.
- [x] Update `site/src/config/site.ts` to `en_AU` and record the approved REQ-9 drift here and in `documents/guides/_hosting.md`.
- [x] Build and confirm all six documents contain exactly one `en_AU` locale; HTML language remains unchanged.

Release verification requires both triggered deployments to succeed, all five staging routes to emit `en_AU`, and production to retain `server: GitHub.com`. This check runs after the scoped GAC and push; it does not authorise cutover.

### Implemented Solution: Open Graph locale correction

- `site/src/config/site.ts`: changed only `openGraphLocale` and its explanatory comment; HTML language is unchanged.
- `site/test/production-parity.test.ts`: changed the expected locale and documented the user-approved exception. The pre-fix regression failed as expected (six passes, one failure: `en-AU` versus `en_AU`).
- `documents/todo/astro_workers_migration_plan.md` and `documents/guides/_hosting.md`: recorded what differs from production, the protocol reason, and explicit user approval. This does not authorise Step 9, DNS changes or a production Worker binding.
...
- [x] Step 10: decommission legacy hosting and files, archive old plan, verify final deployment.
...
Final local gate passed after retirement: check/build zero diagnostics, 61 unit tests, 78 browser cases, six viewport-specific skips; maximum visual difference 0.8362%. Documentation/path checks passed. Step 10 passed the committed retirement build and apex read-back recorded below.

### Final completion evidence

- Retirement source `9f8025e84fe75ea51011672926c1e7ceec5f7fbd` deployed through Workers Build `c104a488-491c-45f1-9166-f4d4842f8228`, successful; active Worker version `eb7905a1-a4a9-4ed0-be2c-838436fa2659` at 100%. The deployed source matched `origin/main` at verification. No legacy workflow remained to run.
- Post-retirement live verification passed again: 18 HTTP contract cases; generated Markdown and conditional requests; all five routes under six browser/crawler user agents with no noindex and exact build bytes; three discovery files; one www 308 preserving path/query; 12 browser route/viewport combinations with zero console, page, CSP, first-party-request or overflow errors.
- Final local gate passed: `pnpm --dir site check`, `build`, `test`; 61 unit tests, 78 browser tests, six viewport-specific skips, maximum visual difference 0.8362%. Local Wrangler contract passed 18/18. Independent extra checks proved FAQ direct/same-page/repeated-hash paths at both widths and mobile contact 500/200 with intercepted four-field payloads.
- All retirement resources and the 12 protected DNS records were read back successfully. Current documents passed synchronisation and source-path checks. Task-owned browsers and local servers are closed. The completion-only documentation revision uses the existing manual Builds trigger when the `site/*` filter skips it, so final live source is rechecked against the final `origin/main` without changing application behaviour.
- Evidence: `documents/guides/parity/step9-cutover-20260905/final-gate.json`, `retirement-deployed.json`, `legacy-retired.json`, `final-browser.json`, `final-http-contract.log`, `indexability.json`, `faq-contact-extra.json`, `lighthouse-summary.json`, and archived raw Lighthouse reports. No required migration check remains skipped; the six suite skips select viewport-specific cases and their applicable/extra checks passed. Search-engine inclusion remains externally controlled and is not claimed; production is verified indexable.
