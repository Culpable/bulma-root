...
test('shared buttons enforce target sizes, exact press feedback, and static opt-out', () => {
  const source = readSiteFile('src/components/elements/button.tsx')
  const globalStyles = readSiteFile('src/styles/global.css')
  const footer = readSiteFile('src/components/sections/footer-with-newsletter-form-categories-and-social-icons.tsx')
...
test('secondary navbar controls enforce the shared responsive target sizes', () => {
  const source = readSiteFile('src/components/sections/navbar-with-links-actions-and-centered-logo.tsx')

  assert.match(source, /aria-label="Open menu"[\s\S]*className="[^"]*size-11/)
  assert.match(source, /aria-label="Close menu"[\s\S]*className="[^"]*size-11/)
...
test('FAQ disclosures and icons use reversible enter and leave contracts', () => {
  ...
  const controller = readSiteFile('src/components/sections/faq-disclosure-controller.tsx')
...
test('mobile navigation exits through transition state before native dialog close', () => {
  const globalStyles = readSiteFile('src/styles/global.css')
  const navbar = readSiteFile('src/components/sections/navbar-with-links-actions-and-centered-logo.tsx')
  const controller = readSiteFile('src/components/sections/navbar-controller.tsx')
...
test('tilt and ripple effects avoid idle compositor promotion and forced layout', () => {
  const screenshot = readSiteFile('src/components/elements/screenshot.tsx')
  const magneticWrapper = readSiteFile('src/components/elements/magnetic-wrapper.tsx')
...
test('surface geometry, media edges, and pricing focus retain comparison clarity', () => {
  const globalStyles = readSiteFile('src/styles/global.css')
  const features = readSiteFile('src/components/sections/features-two-column-with-demos.tsx')
  const pricingPage = readSiteFile('src/components/pages/pricing-sections.tsx')
  const homepage = readSiteFile('src/components/pages/home-sections.tsx')
  const pricingHero = readSiteFile('src/components/sections/pricing-hero-multi-tier.tsx')
  const pricingSection = readSiteFile('src/components/sections/pricing-multi-tier.tsx')
  const main = readSiteFile('src/components/elements/main.tsx')
...
test('Tailwind longhand motion utilities transition the properties they change', () => {
  ...
  assert.match(
    readSiteFile('src/components/sections/call-to-action-simple.tsx'),
    /transition-\[translate,scale,opacity\]/,
  )
  assert.match(readSiteFile('src/components/sections/faqs-accordion.tsx'), /transition-\[translate,opacity\]/)
})
