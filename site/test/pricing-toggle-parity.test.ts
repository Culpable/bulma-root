...
test('homepage and pricing page toggles share every interactive visual token', () => {
  const sharedStyles = readSiteFile('src/components/sections/pricing-toggle-shared.ts')
  const homepagePricing = readSiteFile('src/components/sections/pricing-multi-tier.tsx')
  const pricingPage = readSiteFile('src/components/sections/pricing-hero-multi-tier.tsx')
