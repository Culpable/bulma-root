...
export function NavbarLink({ children, href, className, currentPath = '/', ...props }: ComponentProps<typeof TransitionLink> & { currentPath?: string }) {
 ...
 */
export function NavbarMobileLink({ children, href, className, currentPath = '/', ...props }: ComponentProps<'a'> & { href: string; currentPath?: string }) {
  const isActive = isActivePath(href, currentPath)
